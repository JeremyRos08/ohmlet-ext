...
function addPinPosts(group: THREE.Group, pins: THREE.Vector3[], yTop: number, male = false): void {
  const socketGeo = cachedGeometry('arduino-header-socket-open', () => {
    const shape=new THREE.Shape();shape.moveTo(-.49,-.36);shape.lineTo(.49,-.36);shape.lineTo(.49,.36);shape.lineTo(-.49,.36);shape.closePath()
    ...
    const g=new THREE.ExtrudeGeometry(shape,{depth:.65,bevelEnabled:false});g.translate(0,0,-.325);g.rotateX(-Math.PI/2);return g
  })
  ...
  for (const p of pins) {
    ...
    socket.position.set(p.x, male ? yTop - .66 : yTop - .18, p.z)
    group.add(socket)
    const leg = new THREE.Mesh(legGeo, gold)
    leg.position.set(p.x, male ? yTop - .16 : yTop - .68, p.z)
    group.add(leg)
  }
}
...
function buildNanoLocal(
  ...
  group.add(pcb)
  addPinPosts(group, pins, pcbY + 0.52, true)
  ...
  details.tactile(c.x-3.8,pcbY+.13,c.z)
  details.isp(minX+.5,pcbY+.14,c.z)
  details.finish()
  ...
  const lbl = boardLabel('NANO', 1.6, 0.48)
  if (lbl) {
    lbl.position.set(c.x - 2.2, pcbY + 0.18, c.z)
    group.add(lbl)
  }
...
export function buildArduinoUno(
  ...
  pins.forEach(p=>{details.pad(p.x,pcbY+.15,p.z);details.pad(p.x,pcbY-.15,p.z)})
  for(const a of [-1,1])for(const b of [-1,1])details.pad(bodyCx+a*(bodyW/2-.65),pcbY+.14,bodyCz+b*(bodyD/2-.65),.31)
  details.barrel(bodyCx-bodyW/2-.35,pcbY+.95,bodyCz+2.6)
  details.isp(bodyCx+7.5,pcbY+.14,bodyCz+.4)
  details.tactile(bodyCx-5.3,pcbY+.14,bodyCz+4)
  details.box(bodyCx+1.2,pcbY+.17,bodyCz+.4,6.55,.20,2.55,plastic(0x282a2d,.65))
  details.label('POWER',bodyCx-2,pcbY+.15,bodyCz+3.9,2,.3)
  details.label('ANALOG IN',bodyCx+4.7,pcbY+.15,bodyCz+3.9,2.5,.3)
  details.finish()
